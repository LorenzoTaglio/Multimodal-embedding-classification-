...
def main(args):
    ...
    if args.fusion in ["early", "both"]:
        ...
        ealry_fusion_results.to_dataframe().to_csv(f"out\\{date}_early_fusion_.csv")
        print("Results saved to out\\early_fusion.csv")
 

    # -----------------------------------late-fusion-----------------------------------
    if args.fusion in ["late", "both"]:
        ...
        for key, results in late_fusion_results.items():
            results.to_dataframe().to_csv(f"out\\{date}_late_fusion_{key}.csv")

        late_fusion_meta_results = late_fusion.late_fusion_meta()
        late_fusion_meta_results.to_dataframe().to_csv("out\\late_fusion_meta.csv")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Fusion types. (options: early,late,both) (default: both)"
    )
